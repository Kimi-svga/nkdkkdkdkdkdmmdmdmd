...
"""
SHONEX ULTRA v3.1 — OOP Edition + Full Interface
Сохранён весь функционал: категории, FSM, баланс, заказы, админка, Stars.
Архитектура: классы для БД, хендлеров, клавиатур и оркестратора.
"""
...
class Database:
    ...
    async def freeze_balance(self, uid: int, amount: int) -> None:
        async with self.connection() as conn:
            await conn.execute(
                "UPDATE users SET stars_balance = stars_balance - $1, "
                "stars_frozen = stars_frozen + $2 WHERE user_id = $3",
                amount, amount, uid
            )

    async def release_frozen(self, client_id: int, executor_id: int,
                             amount: int, executor_reward: int) -> None:
        async with self.connection() as conn:
    ...
    async def create_order(self, client_id: int, category: str, service: str,
        ...
        async with self.connection() as conn:
            row = await conn.fetchrow("""
                ...
                RETURNING id
            """, client_id, category, service, description,
    ...
    async def list_available_orders(self) -> List[Dict[str, Any]]:
        async with self.connection() as conn:
            rows = await conn.fetch(
                ...
                "AND executor_id IS NULL ORDER BY id DESC LIMIT 20"
            )
...
class Keyboards:
    """Фабрика всех клавиатур. Возвращает готовые InlineKeyboardMarkup."""
...
class MainHandlers(BaseHandler):
    ...
    async def order_amount(self, message: Message, state: FSMContext):
        ...
        if amount_stars > MAX_STARS_PER_INVOICE:
            await message.answer(f"❌ Сумма превышает лимит Stars ({MAX_STARS_PER_INVOICE}).")
            return
    ...
    # --- Executor Take & Submit (handlers needed for full interface) ---
    ...
    async def cb_executor_submit(self, call: CallbackQuery, state: FSMContext):
        ...
        if not order or order["executor_id"] != call.from_user.id:
            await call.answer("Нет доступа", show_alert=True)
            return
        await state.set_state(OrderState.description)  # reuse
...
class AdminHandlers(BaseHandler):
    ...
    def _register(self):
        ...
        self.router.message(Command("approve"))(self.admin_approve)
        self.router.message(Command("reject"))(self.admin_reject)
    ...
    async def admin_approve(self, message: Message):
        if message.from_user.id not in ADMIN_IDS:
            return
        try:
    ...
    async def admin_reject(self, message: Message):
        if message.from_user.id not in ADMIN_IDS:
            return
        try:
            uid = int(message.text.split("_")[1])
